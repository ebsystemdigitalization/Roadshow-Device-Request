...
import { Users, UserPlus, Search, Edit2, Trash2, Shield, CheckCircle2, X, AlertTriangle, Upload, Download, ChevronLeft, ChevronRight, Eye, EyeOff, KeyRound, Copy, Check } from 'lucide-react';
...
interface AdminUserManagementProps {
  ...
  onSwitchUserPersona: (user: User) => void;
...
export const AdminUserManagement: React.FC<AdminUserManagementProps> = ({
  ...
  const copyPasswordToClipboard = (userId: string, pass: string) => {
  ...
  const handleOpenAddModal = () => {
    ...
    const defaultHOU = users.find(u => u.role === 'Head of Unit')?.name || 'Dr. Kamil Ariffin';
  ...
  const handleOpenEditModal = (u: User) => {
    ...
    setFormData({
      ...
      headOfUnit: u.headOfUnit || (u.role === 'Head of Unit' ? u.name : 'Dr. Kamil Ariffin'),
  ...
  const filteredUsers = users.filter(u => {
    ...
    const matchesSearch =
      ...
      (u.headOfUnit && u.headOfUnit.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const getRoleBadgeStyle = (role: UserRole) => {
    switch (role) {
      ...
      case 'Head of Unit': return 'bg-cyan-50 text-cyan-700 border-cyan-200';
    ...
    <div id="admin-user-management-container" className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl p-6 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1 max-w-xl">
            Configure system users, assign role-based authorization levels (Sales Team, Head of Sales, Head of Unit, Device Team, Head of Operation, Head of Department, Admin) and region assignments.
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse min-w-[1150px] whitespace-nowrap">
            ...
            <tr>
              ...
              <th className="p-3.5">6. Head of Unit</th>
              <th className="p-3.5">7. Head of Sales</th>
              <th className="p-3.5">8. Head of Department</th>
              <th className="p-3.5">9. User Status</th>
              <th className="p-3.5">10. Password</th>
              <th className="p-3.5 text-center">Simulation Persona</th>
              ...
              paginatedUsers.map((u) => {
                ...
                return (
                  <tr key={u.id} className={`hover:bg-slate-50 transition-colors ${u.id === currentUser.id ? 'bg-indigo-50/40 font-medium' : ''}`}>
                    ...
                    <td className="p-3.5 text-cyan-800 font-semibold">{u.headOfUnit || '-'}</td>
                    ...
                    <td className="p-3.5">
                      <div className="inline-flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100/80 border border-slate-200/80 px-2.5 py-1 rounded-lg transition-colors">
                        <KeyRound className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                        <span className="font-mono text-xs text-slate-700 font-medium select-all">
                          {isPasswordRevealed ? userPassword : '••••••••'}
                        </span>
                        <button
                          type="button"
                          onClick={() => togglePasswordVisibility(u.id)}
                          className="text-slate-400 hover:text-slate-700 p-0.5 rounded hover:bg-slate-200/60 transition-colors ml-0.5 cursor-pointer"
                        ...
                        <button
                          type="button"
                          onClick={() => copyPasswordToClipboard(u.id, userPassword)}
                          className="text-slate-400 hover:text-indigo-600 p-0.5 rounded hover:bg-indigo-50 transition-colors cursor-pointer"
                          title="Copy Password"
                        >
                    ...
                    <td className="p-3.5 text-center">
                      <button
                        ...
                        onClick={() => onSwitchUserPersona(u)}
                        disabled={u.id === currentUser.id || currentStatus !== 'Active'}
                        ...
                        title={currentStatus !== 'Active' ? 'Inactive user' : undefined}
                      >
                        {u.id === currentUser.id ? 'Current User' : currentStatus !== 'Active' ? 'Inactive' : 'Switch Persona'}
                      </button>
                    </td>
                  <td className="p-3.5 text-center">
      ...
      {/* Add / Edit User Modal */}
      {isModalOpen && (
        <div id="admin-user-modal-backdrop" className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 my-auto">
              ...
              <div className="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Head of Unit">c. Head of Unit</option>
                    <option value="Device Team">d. Device Team</option>
                    <option value="Head of Operation">e. Head of Operation</option>
                    <option value="Head of Department">f. Head of Department</option>
                    <option value="Admin">g. Admin</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    />
                    <datalist id="head-of-unit-options">
                      {users.filter(u => u.role === 'Head of Unit').map(hou => (
                        <option key={hou.id} value={hou.name}>{hou.name} ({hou.region})</option>
                      ))}
                      <option value="Dr. Kamil Ariffin" />
                    </datalist>
                    <p className="text-[11px] text-slate-400">Select or enter designated Head of Unit responsible for this user.</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Enter user login password (e.g. Password123!)"
                      className="w-full pl-9 pr-10 border border-slate-300 rounded-xl px-3 py-2 text-xs font-mono outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                    <button
                      type="button"
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">Set the login authentication password for this user account.</p>
                </div>
              </div>

              <div className="p-4 px-6 flex items-center justify-end gap-2 border-t border-slate-200 bg-slate-50 shrink-0">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 border border-slate-300 text-slate-700 rounded-xl text-xs font-semibold cursor-pointer hover:bg-slate-100 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  type="submit"
                  className="inline-flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2 rounded-xl text-xs font-semibold shadow-md shadow-indigo-500/20 cursor-pointer transition-all"
                >
