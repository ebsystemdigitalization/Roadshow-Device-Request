...
import { formatRM, formatDate, isRequestForHeadOfSales, isRequestForHeadOfUnit, isRequestForHeadOfDepartment } from '../utils/formatters';
...
export const RequestList: React.FC<RequestListProps> = ({
  ...
  // - Head of Unit only views requests for users tagged to them in User Access Management
  ...
  const roleBasedRequests = requests.filter(r => {
    ...
    if (currentUser.role === 'Head of Unit') {
      return isRequestForHeadOfUnit(r, currentUser, users);
    }
  ...
  const pendingRoleActionRequests = roleBasedRequests.filter(r => {
    ...
    if (currentUser.role === 'Head of Unit') return r.status !== 'Approved' && r.status !== 'Rejected';
  ...
  const getRolePendingText = () => {
    ...
    if (currentUser.role === 'Head of Unit') return 'from users tagged to your unit requiring review or progress';
    ...
    const matchesStatus =
      statusFilter === 'ALL'
        ...
        ? (
            ...
            (currentUser.role === 'Head of Unit' && r.status !== 'Approved' && r.status !== 'Rejected') ||
