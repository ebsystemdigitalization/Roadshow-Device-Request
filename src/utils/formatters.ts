...
export function generateRequestCode(existingRequests?: { requestCode?: string }[] | string[]): string {
  const year = new Date().getFullYear();
  let codesToCheck: string[] = [];

  if (existingRequests && existingRequests.length > 0) {
    codesToCheck = existingRequests.map(r => (typeof r === 'string' ? r : (r?.requestCode || '')));
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) {
          codesToCheck = parsed.map((r: any) => r?.requestCode || '');
        }
      }
    } catch {
      // ignore
  ...
  // Find the highest sequence number for this year or overall
  let maxSeq = 0;
  const yearPrefixRegex = new RegExp(`^RDR-${year}-(\\d+)$`, 'i');
  const genericPrefixRegex = /^RDR-\d{4}-(\\d+)$/i;
  ...
  for (const code of codesToCheck) {
    ...
    // First try matching current year e.g. RDR-2026-0005
    const yearMatch = trimmed.match(yearPrefixRegex);
    if (yearMatch && yearMatch[1]) {
      ...
      // Fallback matching any year's RDR-YYYY-XXXX
      const anyMatch = trimmed.match(genericPrefixRegex) || trimmed.match(fallbackRegex);
      if (anyMatch && anyMatch[1]) {
        const seq = parseInt(anyMatch[1], 10);
        if (!isNaN(seq) && seq > maxSeq) {
          maxSeq = seq;
        }
      }
...
export function normalizeUserRole(roleStr: string): UserRole {
  ...
  if (r.includes('head of unit') || r === 'hou') return 'Head of Unit';
...
export function parseUserCSV(csvText: string, existingUsers: User[] = []): { users: User[]; errors: string[] } {
  ...
  if (isHeader) {
    ...
    const findHou = headers.findIndex(h => h.includes('head of unit') || h.includes('hou') || h.includes('unit'));
    if (findHou !== -1) houIdx = findHou;
    ...
    const findPass = headers.findIndex(h => h.includes('password') || h.includes('pass'));
    if (findPass !== -1) passIdx = findPass;
  }
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    const headOfUnitInput = houIdx !== -1 ? cols[houIdx] : 'Dr. Kamil Ariffin';
    const headOfSalesInput = cols[hosIdx] || "Dato' Wong Wei Sheng";
    const headOfDeptInput = hodIdx !== -1 ? cols[hodIdx] : (cols.length > 7 ? cols[7] : 'Ir. Ahmad Rizal');
    const statusInput = statusIdx !== -1 && cols[statusIdx] ? cols[statusIdx] : (cols.length > 8 ? cols[8] : (cols[7] || 'Active'));
    const passwordInput = passIdx !== -1 && cols[passIdx] ? cols[passIdx] : (cols.length > 9 ? cols[9] : 'Password123!');
    ...
    const headOfUnit = role === 'Head of Unit' ? (headOfUnitInput || name) : (headOfUnitInput || 'Dr. Kamil Ariffin');
...
export function generateSampleUserCSV(): string {
  return `Name, Email, Role, State, Region, Head of Unit, Head of Sales, Head of Department, User Status, Password
Ahmad Razak, ahmad.razak@example.com, Sales Team, Selangor, Central, Dr. Kamil Ariffin, Dato' Wong Wei Sheng, Ir. Ahmad Rizal, Active, Password123!
Siti Sarah, siti.sarah@example.com, Head of Sales, Penang, Northern, Dr. Kamil Ariffin, Siti Sarah, Ir. Ahmad Rizal, Active, Password123!
Dr. Kamil Ariffin, kamil.ariffin@example.com, Head of Unit, Kuala Lumpur, Central, Dr. Kamil Ariffin, Dato' Wong Wei Sheng, Ir. Ahmad Rizal, Active, Password123!
Wong Wei Ming, wong.wm@example.com, Device Team, Kuala Lumpur, Central, Dr. Kamil Ariffin, Dato' Wong Wei Sheng, Ir. Ahmad Rizal, Active, Password123!
Faridah Kassim, faridah.k@example.com, Head of Operation, Johor, Southern, Dr. Kamil Ariffin, Dato' Wong Wei Sheng, Ir. Ahmad Rizal, Active, Password123!
Ir. Ahmad Rizal, ahmad.rizal@example.com, Head of Department, Kuala Lumpur, Central, Dr. Kamil Ariffin, Dato' Wong Wei Sheng, Ir. Ahmad Rizal, Active, Password123!
Johnathan Tan, johnathan.tan@example.com, Sales Team, Sarawak, Sarawak, Dr. Kamil Ariffin, Dato' Wong Wei Sheng, Ir. Ahmad Rizal, Active, Password123!
Nurul Huda, nurul.huda@example.com, Sales Team, Sabah, Sabah, Dr. Kamil Ariffin, Dato' Wong Wei Sheng, Ir. Ahmad Rizal, Active, Password123!`;
}

export function downloadUsersCSV(usersList: User[]): void {
  const headers = ['User ID', 'Name', 'Email', 'Role', 'State', 'Region', 'Head of Unit', 'Head of Sales', 'Head of Department', 'User Status', 'Password'];
...
export function isRequestForHeadOfUnit(
  request: RoadshowRequest,
  headOfUnitUser: User,
  allUsers: User[]
): boolean {
  if (!headOfUnitUser || headOfUnitUser.role !== 'Head of Unit') {
    return true;
  }

  // Head of Unit can view their own created requests
  if (
    request.createdByUserId === headOfUnitUser.id ||
    (request.createdByName && request.createdByName.trim().toLowerCase() === headOfUnitUser.name.trim().toLowerCase())
  ) {
    return true;
  }
  ...
  if (creator) {
    // Return true if the request creator is tagged to this Head of Unit in User Access Management
    const assignedHou = (creator.headOfUnit || '').trim().toLowerCase();
    const currentHouName = (headOfUnitUser.name || '').trim().toLowerCase();
    const currentHouId = (headOfUnitUser.id || '').trim().toLowerCase();
    return assignedHou === currentHouName || assignedHou === currentHouId;
  }

  // Fallback if creator user is not in allUsers table
  return !headOfUnitUser.region || headOfUnitUser.region === 'ALL' || request.partA.region === headOfUnitUser.region;
}
